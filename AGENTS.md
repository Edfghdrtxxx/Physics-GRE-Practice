...
- **Similar-problem control on every flashcard:** `bank.js` owns `PGRE.similarQuestionForCard(card)` (same-topic token overlap, ≥2 shared tokens, default practice pool only — never intact exams), `PGRE.similarProblemButtonHTML` / `similarProblemInlineHTML` (disabled "No similar problem" when nothing qualifies — never an unrelated question), `PGRE.wireSimilarProblemButtons(root)`, and `PGRE.openSimilarProblem` (one-question `pgre-quiz-config` → `#/practice/custom`). Every card surface renders it: Study front row + revealed grade row, session peek, post-Again scaffold, browse peeks, picker previews, search cards (`view-formulas.js`), and Match tiles (inline variant — tiles are already `<button>`s), Type, Quiz, Cloze (`flashmodes.js`). New card surfaces must include it.
