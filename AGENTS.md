...
- **Plan and packs are joined:** plan set tasks are `set-NN`, packs are `NN`. `view-plan.js` / the dashboard "This week" card launch via `PGRE.launchPack(n)`; a finished pack ticks its `set-NN` task through `gamify.toggleTask` (never un-ticks). Agent-receipt UI on the practice summary renders only for pack sessions; `packReceipts[NN]` is still written for the OrbitOS skill.
