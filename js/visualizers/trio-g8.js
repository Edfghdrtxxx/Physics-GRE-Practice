/* Formula visualizers — G8 Delta U / E=-grad V / Poisson V / Bn continuity */
(function (global) {
  'use strict';
...
The minus sign is the whole story. When $\\mathbf{F}$ does positive work, $U$ falls. Climbing against $\\mathbf{F}$ stores $U$. Because $\\nabla\\times\\mathbf{F}=0$, every path from $A$ to $B$ returns the same $\\Delta U$. GRE integrals therefore collapse onto axis-aligned (Manhattan) segments: $\\Delta U = -\\int F_x\\,dx - \\int F_y\\,dy$.
    ...
    draw: function (ctx, width, height, state, dt) {
      ...
      appendLegend('$\\Delta U = -\\int \\mathbf{F}\\cdot d\\mathbf{l}$', [
        ...
        { label: '$U(A)$', value: money(uA) },
        { label: '$U(B)$', value: money(uB) },
        { label: '$\\Delta U$', value: money(deltaU) },
        { label: '$W_{\\mathrm{field}}(A\\to B)$', value: money(Wfull) },
        { label: '$W(s)$ running', value: money(Wrun) },
        { label: 'Check', value: '$\\Delta U + W = 0$ on every path' }
      ]);
    },
    challenge: {
...
A GRE classic: midway between $+Q$ and $+Q$, $\\mathbf{E}=0$ but $V\\neq 0$. Midway between $+Q$ and $-Q$, $V=0$ but $\\mathbf{E}\\neq 0$. Slope, not height.
    ...
    parameters: [
      ...
      { id: 'chargeMag', label: 'Strength $q$ / $E_0$', min: 0.5, max: 2.5, step: 0.1, default: 1.0 },
      { id: 'simSpeed', label: 'Simulation Speed', min: 0.2, max: 3.0, step: 0.2, default: 1.0, unit: 'x' }
    ],
    init: function (container, state) {
    ...
    draw: function (ctx, width, height, state, dt) {
      ...
      appendLegend('$\\mathbf{E} = -\\nabla V$', [
        { label: 'Config', value: preset === 'capacitor' ? 'parallel plates' : (preset === 'dipole' ? 'dipole' : (preset === 'like_charges' ? 'like charges' : 'point charge')) },
        { label: '$V$', value: money(vP) },
        { label: '$|\\mathbf{E}|$', value: money(eP.mag) },
        { label: 'Geometry', value: trapNote }
      ]);
    },
    challenge: {
...
For a uniformly charged sphere the integral is Newton's shell theorem. Outside, $V=kQ/r$. Inside a solid sphere $V$ is a downward parabola with $V(0)=\\frac{3}{2}V(R)$. Inside a thin shell $V$ is strictly constant (Faraday cage). On the axis of a ring every $dq$ is equidistant, so $V(z)=kQ/\\sqrt{R^2+z^2}$.
    ...
    parameters: [
      ...
      { id: 'radius', label: 'Radius $R$', min: 0.20, max: 0.55, step: 0.05, default: 0.35 },
      { id: 'totalCharge', label: 'Total charge $Q$', min: 0.4, max: 2.4, step: 0.1, default: 1.0 },
      { id: 'simSpeed', label: 'Simulation Speed', min: 0.2, max: 3.0, step: 0.2, default: 1.0, unit: 'x' }
    ],
    init: function (container, state) {
    ...
    draw: function (ctx, width, height, state, dt) {
      ...
      appendLegend('Poisson integral for $V$', [
        { label: 'Source', value: geoName },
        { label: isRing ? '$z$' : '$r$', value: money(rP, 3) + ' (' + region + ')' },
        { label: '$V$', value: money(Vp) },
        { label: '$|\\mathbf{E}|$', value: money(Ep) },
        { label: 'point-charge $kQ/r$', value: money(Vcoul) },
        { label: 'Match', value: (!isRing && !inside) ? 'outside: $V=kQ/r$' : (geo === 'sphere_solid' && inside ? '$V(0)=\\frac{3}{2}V(R)$' : (geo === 'sphere_shell' && inside ? 'cage: $V=kQ/R$, $E=0$' : 'on axis: $V=kQ/\\sqrt{R^2+z^2}$')) }
      ]);
    },
    challenge: {
...
With $\\mathbf{K}_f=0$, $B_\\parallel$ scales with $\\mu$, so $\\tan\\theta_2/\\tan\\theta_1=\\mu_2/\\mu_1$.
    ...
    parameters: [
      { id: 'bField', label: 'Incident $|\\mathbf{B}_1|$', min: 1.0, max: 5.0, step: 0.5, default: 3.0, unit: 'T' },
      { id: 'thetaIn', label: 'Incident angle $\\theta_1$', min: 0, max: 70, step: 5, default: 35, unit: 'deg' },
      { id: 'muRatio', label: 'Permeability $\\mu_2/\\mu_1$', min: 0.3, max: 6.0, step: 0.1, default: 2.4 },
      { id: 'surfaceK', label: 'Free current $\\mu_1 K_f$', min: -2.0, max: 2.0, step: 0.25, default: 0.0, unit: 'T' },
      { id: 'simSpeed', label: 'Simulation Speed', min: 0.2, max: 3.0, step: 0.2, default: 1.0, unit: 'x' }
    ],
    init: function (container, state) {
    ...
    draw: function (ctx, width, height, state, dt) {
      ...
      appendLegend('$B^\\perp$ is continuous', [
        { label: '$B_{\\mathrm{in}}^\\perp$', value: '$' + B1perp.toFixed(2) + '\\,\\mathrm{T}$' },
        { label: '$B_{\\mathrm{out}}^\\perp$', value: '$' + B2perp.toFixed(2) + '\\,\\mathrm{T}$' },
        { label: '$\\Delta B^\\perp$', value: '$0$ (always)' },
        { label: '$B_{\\mathrm{in}}^\\parallel$', value: '$' + B1par.toFixed(2) + '\\,\\mathrm{T}$' },
        { label: '$B_{\\mathrm{out}}^\\parallel$', value: '$' + B2par.toFixed(2) + '\\,\\mathrm{T}$' },
        { label: '$\\Delta B^\\parallel$', value: '$' + (B2par - B1par).toFixed(2) + '\\,\\mathrm{T}$' },
        { label: '$\\theta_1 \\to \\theta_2$', value: '$' + thetaInDeg.toFixed(0) + '^\\circ \\to ' + thetaOutDeg.toFixed(0) + '^\\circ$' },
        { label: '$\\mu_2/\\mu_1$', value: money(muRatio, 1) }
      ]);
    },
    challenge: {
